...
function initBall() {
    ...
    ball = new Ball(new Rectangle(BALL_SIZE, BALL_SIZE,
                                  INITIAL_BALL_POSITION.x, INITIAL_BALL_POSITION.y,
                                  "#FFFFFF"),
                    new Point(initialBallVelocity.x, initialBallVelocity.y));
}

function initPlayers() {
    player1 = new Player(new Rectangle(PADDLE_DIMENSIONS.x, PADDLE_DIMENSIONS.y,
                                       INITIAL_PLAYER_1_POSITION.x, INITIAL_PLAYER_1_POSITION.y,
                                       "#FFFFFF"),
                         "Player 1", PLAYER_1_UP_KEY, PLAYER_1_DOWN_KEY, BACKGROUND_DIMENSIONS);
    player2 = new Player(new Rectangle(PADDLE_DIMENSIONS.x, PADDLE_DIMENSIONS.y,
                                       INITIAL_PLAYER_2_POSITION.x, INITIAL_PLAYER_2_POSITION.y,
                                       "#FFFFFF"),
...
function initScores() {
    player1Score = new Text(player1.score, TEXT_SIZE, "sans-serif", "#FFFFFF",
                            new Point(BACKGROUND_DIMENSIONS.x * SCORE_PADDING_SCALE.x - TEXT_SIZE,
                                      BACKGROUND_DIMENSIONS.x * SCORE_PADDING_SCALE.y));
    player2Score = new Text(player2.score, TEXT_SIZE, "sans-serif", "#FFFFFF",
                            new Point(BACKGROUND_DIMENSIONS.x - BACKGROUND_DIMENSIONS.x * SCORE_PADDING_SCALE.x,
                                      BACKGROUND_DIMENSIONS.x * SCORE_PADDING_SCALE.y));
}
...
function processPositionOf(ball) {
    ...
    if (ball.body.position.y < 0
        || ball.body.position.y + BALL_SIZE >= BACKGROUND_DIMENSIONS.y) {
        ball.velocity.y *= -1;
    }
...
function nextRound() {
    ...
    ball.body.position      = new Point(INITIAL_BALL_POSITION.x,
                                        INITIAL_BALL_POSITION.y);
    ball.velocity           = new Point(initialBallVelocity.x,
